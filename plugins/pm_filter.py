...
async def cb_handler(client: Client, query: CallbackQuery):
        ...
        if chat_type == enums.ChatType.PRIVATE:
            ...
            if grpid is not None:
                ...
                try:
                    ...
                    return await query.answer('Mallu cinemas')
            else:
    ...
    if query.data.startswith("file"):
        ...
        try:
            if (AUTH_CHANNEL or REQ_CHANNEL) and not await is_subscribed(client, query):
                ...
                await client.send_photo(chat_id=query.from_user.id, photo='https://telegra.ph/file/5b955017ee87740cd3dac.jpg', caption = f"<b>📕Nᴀᴍᴇ ➠ : <code>{files.file_name}</code> \n\n🔗Sɪᴢᴇ ➠ : {get_size(files.file_size)}</b>", 
    ...
    await query.answer('Mallu cinemas')
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb and imdb.get('poster'):
        try:
            ...
            i=await message.reply_photo(photo=poster, caption=cap[:1024], reply_markup=InlineKeyboardMarkup(btn))
            await asyncio.sleep(900)
            await i.delete()
        except Exception as e:
            logger.exception(e)
            t= await message.reply_text(cap, reply_markup=InlineKeyboardMarkup(btn))  
            await asyncio.sleep(900)
            await t.delete()
    else:
        q=await message.reply_text(cap, reply_markup=InlineKeyboardMarkup(btn))
        await asyncio.sleep(900)
        await q.delete()
        await msg.delete()
